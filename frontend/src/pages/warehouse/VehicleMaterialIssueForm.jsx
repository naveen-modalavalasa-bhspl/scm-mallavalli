...
const VehicleMaterialIssueForm = () => {
  ...
  const backPath = '/warehouse/vehicle-material-issues';
  ...
  const activeTreeRow = issueItems.find(r => r.key === activeRowKey);
  ...
  const handleVehicleChange = async (val) => {
    ...
    if (matched) {
      ...
      try {
        const res = await api.get('/masters/vehicles', { params: { search: val, limit: 10 } });
        const data = res.data || [];
        const exactMatch = data.find((v) => v.vehicle_code === val);
        if (exactMatch) {
          form.setFieldsValue({ vehicle_number: exactMatch.vehicle_number });
        }
      } catch (err) {
        console.error('Error fetching vehicle details in change handler:', err);
      }
    }
  };
  ...
  const prefillFromIndent = useCallback(async (indentId) => {
    ...
    try {
      ...
      if (ind.vehicle_code && !ind.vehicle_number) {
        handleVehicleChange(ind.vehicle_code);
      }
  ...
  const fetchRecord = useCallback(async () => {
    ...
    try {
      ...
      // Fetch associated acknowledgement if acknowledged/completed
      if (data.status === 'acknowledged' || data.status === 'completed') {
        try {
          const ackRes = await api.get('/indent/material-acknowledgements', { params: { search: data.issue_number } });
          const acks = ackRes.data?.items || ackRes.data || [];
          if (acks.length > 0) {
            setAssociatedAck(acks[0]);
          }
        } catch (err) {
  ...
  const handleCancel = async () => {
    try {
      await api.post(`/warehouse/vehicle-issues/${id}/cancel`);
      message.success('Vehicle issue cancelled successfully');
      fetchRecord();
    } catch (err) {
      message.error(getErrorMessage(err));
    }
    ...
    try {
      ...
      if (insufficientStock.length > 0) {
        const msgs = insufficientStock.map((i) => {
          const available = stockMap[i.item_id] ?? 0;
          return `Insufficient stock for item ${i.item_code || i.item_id}: available=${available}, requested=${i.qty}`;
        }).join(' ; ');
        message.error(msgs);
        return;
      }
  ...
  const handleItemSelect = async (selectedItems) => {
    const warehouseId = form.getFieldValue('warehouse_id');
    const newItems = selectedItems.map((it) => {
      const defaultRate = Number(it.valuation_rate) || Number(it.purchase_price) || 0;
      return {
        key: Date.now() + Math.random(),
        item_id: it.id,
        item_name: it.name,
    ...
    setIssueItems((prev) => {
      const filtered = prev.filter((i) => i.item_id !== null);
      return [...filtered, ...newItems];
    });

    if (warehouseId) {
      const ids = selectedItems.map((it) => it.id);
      await refreshStockForItems(warehouseId, ids);
      ids.forEach((itemId) => fetchItemStockDetails(warehouseId, itemId));
    }
  };
        ...
        <head>
          ...
          <style>
            ...
            @media print {
              ...
              .label-card { border: 1px solid #000000; }
            }
          </style>
        </head>
        <body>
  ...
  const viewItemColumns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        if (!ackItem || !ackItem.photos || ackItem.photos.length === 0) return '-';
        return (
          <Image.PreviewGroup>
            <Space wrap size={4}>
  ...
  const issueItemColumns = [
    ...
    },
    {
      ...
      render: (val, record) => {
        ...
        const selectedBatchIds = new Set(record.batch_ids || (record.batch_id ? [record.batch_id] : []));
                ...
                >
                  <Select
                    ...
                    onChange={async (val) => {
                      ...
                      setRateMap({});
                      try {
                        const whRes = await api.get(`/masters/warehouses/${val}`);
                        const whData = whRes.data;
                        setIsCentralWarehouse(
                          !!(whData?.is_central ?? (whData?.parent_id === null || whData?.parent_id === undefined))
                        );
                      ...
                      const itemIds = issueItems.map((i) => i.item_id).filter(Boolean);
                      refreshStockForItems(val, itemIds);
                      itemIds.forEach((id) => fetchItemStockDetails(val, id));
                    }}
                  />
                </Form.Item>
              </Col>
              ...
              <Col xs={24} sm={12} md={8}>
                <Form.Item label="Reference Indent" name="indent_id">
                  <Select
                    ...
                    onChange={(val) => prefillFromIndent(val)}
            ...
            <Table
              ...
              summary={() => (
                <Table.Summary.Row>
                  ...
                  <Table.Summary.Cell index={2}><strong>{formatNumber(calcTotalQty())}</strong></Table.Summary.Cell>
...
export default VehicleMaterialIssueForm;
